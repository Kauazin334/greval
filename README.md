...
Before starting the backend, copy `backend/.env.example` to `backend/.env`, set a
unique bcrypt password hash and a random `SESSION_SECRET` of at least 32 characters.
Set `CORS_ORIGINS` only to the real frontend origins. In production use HTTPS,
`ENVIRONMENT=production`, and `COOKIE_SECURE=true`.

The application now requires staff login for every player record, attachment and
download. Session cookies are httpOnly and SameSite=Strict; state-changing requests
also require the CSRF cookie/header pair. Keep MongoDB bound to a private network and
do not run the Vite development server as the production web server.
